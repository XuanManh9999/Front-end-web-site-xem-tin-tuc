                ...
                Array.from({ length: 5 }).map((_, index) => (
                  <TableRow key={index}>
                    ...
                    {columns.map((col) => (
                      <TableCell
                        key={col.key as string}
                        className="px-5 py-3 text-sm text-gray-500 dark:text-gray-300 min-w-[120px]">
                        <Skeleton width="100%" height={28} />
                      </TableCell>
                    ))}
                    {hasActionColumn && (
                      <TableCell className="flex items-center justify-center gap-2 px-5 py-3 h-full">
                  ...
                  <TableRow key={item.id}>
                    ...
                    {columns.map((col) => (
                      <TableCell
                        ...
                        {col.render ? col.render(item[col.key]) : item[col.key] as string}
                      </TableCell>
                    ))}
                    {hasActionColumn && (
